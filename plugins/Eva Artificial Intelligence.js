...
Cyber.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {
    if (message.message.startsWith('Eva') && eva_functionality !== 'true') {        
        ...
        let acc = os.userInfo().homedir.split('Whats')[1].split('Cyber/')[0] == 'Cyber' ? '7d57838203msh0c5cf65c90a7231p13b461jsn77c8cfa55871' : '7d57838203msh0c582jak19865261js1229n77c8cfa55871'
        let aitalk_mode = ''
        ...
        var payload_client = await WhatsCyberStack.get_eva_ai(encodeURIComponent(trmsg), 'Eva', 'FaridDadashzade', 'WhatsCyber', unique_ident)
        var fins = ''                           
        if (conf.LANG !== 'EN') {
            ...
            if (message.jid.includes('-') && (message.mention !== false && message.mention.length !== 0)) {
                message.mention.map(async (jid) => {
                    if (message.client.user.jid.split('@')[0] === jid.split('@')[0]) {
                        ...
                        var payload_client = await WhatsCyber.get_eva_ai(encodeURIComponent(trmsg), 'Eva', 'FaridDadashzade', 'WhatsCyber', unique_ident)
                        var fins = ''                           
                        if (conf.LANG !== 'EN') {
                            ceviri = await translatte(payload_client.result, {from: 'auto', to: conf.LANG});
                            if ('text' in ceviri) {
                ...
                if (message.reply_message.jid.split('@')[0] === message.client.user.jid.split('@')[0]) {
                    ...
                    unique_ident = message.client.user.jid.split('@')[0] + 'PUBLIC' + message.data.participant.split('@')[0]
                    let acc = os.userInfo().homedir.split('Whats')[1].split('Cyber/')[0] == 'Cyber' ? '7d57838203msh0c5cf65c90a7231p13b461jsn77c8cfa55871' : '7d57838203msh0c582jak19865261js1229n77c8cfa55871'
                    var ainame = os.userInfo().homedir.split('Whats')[1].split('Cyber/')[0]
                    if (ainame !== 'Cyber') return;
                    var finm = message.message.replace('Eva', '').replace(' ', '').replace('@' + message.client.user.jid.split('@')[0], '')
                    var ldet = lngDetector.detect(finm)
                    var trmsg = ''
...
Cyber.addCommand({ pattern: 'vtalk$', desc: vtalk_dsc, fromMe: wk }, (async (message, match) => {
    ...
    try {
        ...
        convertToWav(file)
            .on('end', async () => {
                ...
                var ainame = os.userInfo().homedir.split('Whats')[1].split('Cyber/')[0]
                if (ainame !== 'Cyber') return;
                var payload_client = await WhatsCyberStack.get_eva_ai(encodeURIComponent(ssc), 'Eva', 'FaridDadashzade', 'WhatsCyber', unique_ident)
                var fins = ''                           
                if (conf.LANG !== 'EN') {
                    ceviri = await translatte(payload_client.result, {from: 'auto', to: conf.LANG});
                    if ('text' in ceviri) {
...
Cyber.addCommand({ pattern: 'fulleva ?(.*)', desc: fulleva_dsc, fromMe: true, usage: '.fulleva on / off' }, (async (message, match) => {
