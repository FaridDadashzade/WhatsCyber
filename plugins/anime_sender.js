...
Cyber.addCommand({pattern: 'animepic ?(.*)', fromMe: wk, desc: pic, usage: 'animepic normal // animepic nsfw'}, (async (message, match) => {

  if (match[1] == 'normal') {

    var image_link = await WhatsCyberStack.anime_wall('normal')

    var image_buffer = await axios.get(image_link, { responseType: 'arraybuffer' })

    await message.sendMessage(Buffer.from(image_buffer.data), MessageType.image, { mimetype: Mimetype.png, caption: 'Made by WhatsCyber' })

  } else if (match[1] == 'nsfw') {

    var image_link_nsfw = await WhatsCyberStack.anime_wall('nsfw')

    var image_buffer_nsfw = await axios.get(image_link_nsfw, { responseType: 'arraybuffer' })

    await message.sendMessage(Buffer.from(image_buffer_nsfw.data), MessageType.image, { mimetype: Mimetype.png, caption: 'Made by WhatsCyber' })
...
Cyber.addCommand({pattern: 'animegif ?(.*)', fromMe: wk, desc: giff, usage: 'animegif normal // animegif nsfw'}, (async (message, match) => {

  if (match[1] == 'normal') {

    var gif_link = await WhatsCyberStack.anime_gif('normal')
    ...
    await download(gif_link, '/root/WhatsCyber/pic.mp4', async() => {                          

      await message.client.sendMessage(message.jid, fs.readFileSync('/root/WhatsCyber/pic.mp4'), MessageType.video, { caption: 'Made by WhatsCyber', mimetype: Mimetype.gif })

    })

  } else if (match[1] == 'nsfw') {

    var gif_link_nsfw = await WhatsCyberStack.anime_gif('nsfw')
    ...
    await download(gif_link_nsfw, '/root/WhatsCyber/picn.mp4', async() => {                          

      await message.client.sendMessage(message.jid, fs.readFileSync('/root/WhatsCyber/picn.mp4'), MessageType.video, { caption: 'Made by WhatsCyber', mimetype: Mimetype.gif })
